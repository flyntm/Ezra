...
# 🔥 FULL stderr suppression (kills ONNX C++ warnings)
...
# 🔇 Suppress ONNX spam during model load
...
def get_audio():
    try:
        ...
        print("⚠️ Bad audio chunk, skipping...")
        return None
...
def transcribe(audio):
    ...
    # 🔇 Suppress ONNX spam during transcription too
    with suppress_stderr_fully():
...
def ask_ezra(text):
    response = client.responses.create(
        model="gpt-4.1-mini",
        input=f"Respond conversationally in 1-2 short sentences. Sound natural and warm.\nUser: {text}"
    )
...
def speak(text):
    ...
    os.system(cmd)
    os.system("aplay -D plughw:2,0 temp.wav > /dev/null 2>&1")
