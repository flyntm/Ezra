...
class CommandTimingTests(unittest.TestCase):
    def test_report_lists_pipeline_stages_and_total(self):
        ...
        timing.mark("command_speech_ended", 11.0)
        ...
        self.assertIn("Wake detected → command speech ended: 1.000s", report)
        self.assertIn("Command speech ended → capture complete: 1.000s", report)
        self.assertIn("Speech-to-text: 0.500s", report)
        self.assertIn("Speech synthesis/preparation: 0.200s", report)
        self.assertIn("Command ready → speech started: 0.600s", report)
