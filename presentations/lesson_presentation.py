"""Runtime controller for the current lesson PowerPoint presentation."""
...
# original deck. Runtime presentation startup uses discover_presentation().
DECK_PATH = PRESENTATIONS_DIR / "Lesson_One_Acts.pptx"


def discover_presentation(directory=PRESENTATIONS_DIR):
    """Return the only PowerPoint deck in the presentation directory."""

    presentation_dir = Path(directory)
    decks = sorted(
        path
        for path in presentation_dir.iterdir()
        if path.is_file() and path.suffix.casefold() == ".pptx"
    )
    if not decks:
        raise PresentationError(
            f"No PowerPoint presentation was found in {presentation_dir}."
        )
    if len(decks) > 1:
        names = ", ".join(path.name for path in decks)
        raise PresentationError(
            "More than one PowerPoint presentation was found. "
            f"Please leave exactly one .pptx file in {presentation_dir}. "
            f"Found: {names}"
        )
    return decks[0]
...
_EXPLAIN_SUFFIX_PATTERN = re.compile(
    r"\s+and\s+(?:explain|its|it's)\s*[.!?]?$", re.IGNORECASE
)
...
_SLIDE_JUMP_PATTERN = re.compile(
    ...
    r"[a-z]+(?:[\s-]+[a-z]+){0,2})"
    r"|(?:show|display)\s+(?:us\s+)?(?:the\s+)?"
    r"(?P<number_before>\d{1,3}(?:st|nd|rd|th)?|"
    r"[a-z]+(?:[\s-]+[a-z]+){0,2})\s+slide"
    r")"
    r"(?:\s+(?:of|in|from)\s+(?:the\s+)?presentation)?"
    r"(?:\s+please)?[.!?]?$",
    re.IGNORECASE,
)
_SMALL_NUMBER_WORDS = {
...
def _normalize_presentation_command(command):
    """Correct narrow STT errors only in clear slide-navigation phrases."""
    text = str(command)
    text = re.sub(
        r"\bsign\b(?=\s+(?:number\s+)?(?:\d|one\b|two\b|three\b|four\b|"
        r"five\b|six\b|seven\b|eight\b|nine\b|ten\b))",
        "slide",
        text,
        flags=re.IGNORECASE,
    )
...
class LessonPresentationSession:
    ...
    def _speak_current(self):
        ...
        script = self.deck.notes[self.slide_index]
        if not script.strip():
            print(f"[presentation] slide={slide_number} speaker_notes=none")
            playback_complete.set()
            interrupted = False
        else:
            try:
...
def print_rehearsal(deck, output=print, width=88):
    """Print a quiet, readable review of every slide's narration."""
    rule = "=" * width
    output(rule)
    output("PRESENTATION REHEARSAL")
    output(f"{deck.path.name} — {deck.slide_count} slides")
    output(rule)

    for slide_number, script in enumerate(deck.notes, 1):
        output("")
        label = f"SLIDE {slide_number}"
        if deck.reveal_slides[slide_number - 1]:
            label += " — ANSWER"
        output(label)
        output("-" * len(label))

        cleaned = script.strip()
        if cleaned and deck.auto_advance[slide_number - 1]:
            cleaned = f"{cleaned} [NEXT SLIDE]"
        if not cleaned:
            output("(No speaker notes.)")
            continue
        ...
        for index, paragraph in enumerate(paragraphs):
            if index:
                output("")
            output(textwrap.fill(paragraph, width=width))
    output("")
    output(rule)
    output("END OF REHEARSAL")
    output(rule)
...
def start_presentation(speak, rehearsal=False, slide_number=1, narrate=True):
    ...
    if has_active_presentation():
        _session.stop()
    if rehearsal:
        deck = PowerPointDeck.load(discover_presentation())
        print_rehearsal(deck)
        _session = None
        return

    backend = None
    rehearsal_speak = speak
    look_targets = None
    center_head = None
    ...
    _session = LessonPresentationSession(
        rehearsal_speak,
        backend,
...
def handle_active_command(command, speak):
    global _session
    command = _normalize_presentation_command(command)
...
def rehearse_all():
    print_rehearsal(PowerPointDeck.load(discover_presentation()))
